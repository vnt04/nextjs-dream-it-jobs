...
import { fetchJobs, fetchJobsResult } from "@/lib/data";
...
export default async function ListJobs({
  ...
  const jobsResult = searchParams ? await fetchJobsResult(searchParams) : jobs;
  return (
    <div className="container">
      <div className="py-2 font-semibold">
        Tìm thấy{" "}
        <span className="font-bold text-primary"> {jobsResult?.length} </span>{" "}
        công việc phù hợp với bạn{" "}
        <span className="font-bold text-primary">{searchParams?.tech}</span>
      </div>
